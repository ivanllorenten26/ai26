...
⚠ **Blocked — requires expert input before writing rules or recipes.** Rules in this group cannot be derived from first principles alone; they reflect team-specific decisions, platform constraints, and hard-won production experience. Each item needs a conversation with the relevant expert team before AI26 can codify it.

| # | Rule category | Recipe | Blocked on |
|---|--------------|--------|------------|
| 3 | Security rules (S-\*): no secrets in source, validate at boundary, parameterised queries | `docs/coding-standards/recipes/security.md` | Security team — agreed rule set, threat model, N26-specific constraints |
| 4 | Performance rules (P-\*): no N+1 queries, paginate list endpoints, index FKs | `docs/coding-standards/recipes/performance.md` | Backend/SRE — DB conventions, pagination contract, acceptable query patterns |
| 5 | Observability rules (O-\*): one business metric per use case, structured logging only | — | SRE/Platform — logging format, metric naming, alerting conventions |
| 6 | — | `docs/coding-standards/recipes/caching.md` | Platform — approved caching technologies, TTL policy, invalidation patterns |
| 7 | — | `docs/coding-standards/recipes/ci-cd.md` | Platform/DevOps — CI pipeline structure, deployment conventions, gate criteria |
| 8 | — | `docs/coding-standards/recipes/configuration.md` | Platform — config management approach, secrets injection, feature flag conventions |
| 9 | — | `docs/coding-standards/recipes/scheduling.md` | Backend — approved scheduler (Spring `@Scheduled` vs Quartz vs external), distributed lock approach |
...
| 16 | ~~Fidelity 1 (quick fix) skill~~ | ✅ Already exists as `ai26-implement-fix` (Flow C) |
| 17 | Adoption metrics | ⚠ Phase 1 done: `validation-report.json` + `review-report.json` persisted per ticket, `ai26-promoted` Jira label on promote, `docs/ai26-sdlc/reference/adoption-metrics.md` reference doc. Phase 2 (aggregation script) deferred until ≥ 20 tickets have reports. |

### Group D — CI integration

⚠ **Blocked — PLE team is working on this.**

| # | Item | Description | Blocked on |
|---|------|-------------|------------|
| 18 | GitHub Actions for validation scripts | Wire `scripts/validate-coding-rules.sh` and `scripts/generate-coding-rules-doc.sh --check` into the CI workflow | PLE — coordinate to avoid duplication |
